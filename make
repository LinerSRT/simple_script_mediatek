...
cd $OUT_FOLDER_IMG
rm -rf *.img
mv $PATH_CARLIV/output/"$IMG_NAME"_repacked.img $OUT_FOLDER_IMG/$"$IMG_NAME"_test$NUMBER.img
fi
#####################
#Start building main#
echo -e "${txtbld}**********************************************************************************************${txtrst}"
...
mv output/"$IMG_NAME"_repacked.img $OUT_FOLDER_IMG/"$IMG_NAME"_test$NUMBER.img
BUILD_END=$(date +"%s")
